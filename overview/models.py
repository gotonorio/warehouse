...
class OverView(models.Model):
    ...
    constraction = models.CharField(
        verbose_name="施工会社", max_length=32, blank=True, null=True
    )
    seller = models.CharField(verbose_name="分譲会社", max_length=32, blank=True, null=True)
    management = models.CharField(
        verbose_name="管理会社", max_length=32, blank=True, null=True
    )
    # 法規制
    youto_chiiki = models.CharField(
        verbose_name="用途地域", max_length=32, blank=True, null=True
    )
    specified_floor_area_ratio = models.FloatField(
        verbose_name="指定容積率", blank=True, null=True
    )
    bouka_chiiki = models.CharField(
        verbose_name="防火地域", max_length=32, blank=True, null=True
    )
    specified_building_coverage_ratio = models.FloatField(
        verbose_name="指定建ぺい率", blank=True, null=True
    )
    height_limit = models.CharField(
        verbose_name="高度制限", max_length=32, blank=True, null=True
    )
    required_parking = models.CharField(
        verbose_name="必要駐車場台数", max_length=32, blank=True, null=True
    )
    # 設備概要
    ...
    trunk_room = models.CharField(
        verbose_name="トランクルーム", max_length=32, blank=True, null=True
    )
    delivery_box = models.CharField(
        verbose_name="宅配ボックス", max_length=32, blank=True, null=True
    )
    network = models.CharField(
        verbose_name="インターネット", max_length=32, blank=True, null=True
    )
    entrance = models.CharField(
        verbose_name="共用部エントランス", max_length=32, blank=True, null=True
    )
    security = models.CharField(
        verbose_name="防犯カメラ", max_length=32, blank=True, null=True
    )
...
class RoomType(models.Model):
    ...
    def total_kanrihi(cls, kanrihi_name):
        ...
        """
        qs = cls.objects.annotate(total=F("number_unit") * F(kanrihi_name)).aggregate(
            Sum("total")
        )
        return qs
...
class Room(models.Model):
    ...
    room_type = models.ForeignKey(
        RoomType, verbose_name="住戸タイプ", on_delete=models.PROTECT
    )
    ...
    zip_code = models.CharField(
        verbose_name="郵便番号", max_length=7, blank=True, null=True
    )
    prefecture = models.CharField(
        verbose_name="都道府県", max_length=32, blank=True, null=True
    )
    municipality = models.CharField(
        verbose_name="市区町村", max_length=128, blank=True, null=True
    )
    house_number = models.CharField(
        verbose_name="番地", max_length=64, blank=True, null=True
    )
    building = models.CharField(
        verbose_name="建物名", max_length=64, blank=True, null=True
    )
